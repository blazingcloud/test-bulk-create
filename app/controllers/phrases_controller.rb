class PhrasesController < ApplicationController
  ...
  def new
    @phrases = [Phrase.new,Phrase.new,Phrase.new]
  ...
  # POST /phrases.json
  def create
    @phrases = Phrase.create(params[:phrase])
    respond_to do |format|
      if @phrases.map(&:valid?).uniq == [true]
        format.html { redirect_to phrases_path, notice: 'ALL PHRASES CREATED' }
        format.json { render json: @phrases, status: :created, location: @phrases }
      else
